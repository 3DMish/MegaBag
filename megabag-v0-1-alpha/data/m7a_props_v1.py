...
from bpy_sys import *;
from m7a_text_editor import bl_conf;
...
import m7a_text_editor, m7a_dope_sheet, m7a_view3d_panels;
...
class M7A_MEGABAG_Button(Operator):
    ...
    value  = FloatProperty(min=0.0, default=0.1);
    data   = StringProperty();
    ...
    def execute(self, context):
        ...
        if (self.option == "MOD_APPLY_ALL"):
            for modifier in a_obj.modifiers:
                bpy.ops.object.modifier_apply(apply_as='DATA', modifier=modifier.name);
        
        elif (self.option == "MOD_VIEWPORT_ON"):
            for modifier in a_obj.modifiers: modifier.show_viewport = True;
                
        elif (self.option == "MOD_VIEWPORT_OFF"):
            for modifier in a_obj.modifiers: modifier.show_viewport = False;
                
        elif (self.option == "MOD_VIEWPORT_TOGGLE"):
            for modifier in a_obj.modifiers:
                if (modifier.show_viewport == True):
                    modifier.show_viewport = False;
                else: modifier.show_viewport = True;
        
        elif (self.option == "MOD_RENDER_ON"):
            for modifier in a_obj.modifiers: modifier.show_render = True;
                
        elif (self.option == "MOD_RENDER_OFF"):
            for modifier in a_obj.modifiers: modifier.show_render = False;
                
        elif (self.option == "MOD_RENDER_TOGGLE"):
            for modifier in a_obj.modifiers:
                if (modifier.show_render == True):
                    modifier.show_render = False;
                else: modifier.show_render = True;
                    
        elif (self.option == "MOD_REMOVE_VIEW"):
            list_modifiers = [];
            for modifier in a_obj.modifiers:
                if (modifier.show_viewport == False): list_modifiers.append(modifier.name);
            for modifier in list_modifiers: a_obj.modifiers.remove(a_obj.modifiers[modifier]);
            ...
            for i in range(0, len(a_obj.modifiers)):
                a_obj.modifiers.remove(a_obj.modifiers[len(a_obj.modifiers)-1]);
        
        elif (self.option == "TEXT_REPLACE_ALL"):
            text = lc_space_data.text.as_string();
            text = text.replace(lc_space_data.find_text, lc_space_data.replace_text);
            lc_space_data.text.clear();
            lc_space_data.text.write(text);
            
        
        # -------------------------------------------
        
        elif (self.option == "SET_MISHKEY_PROPS"):
            bpy_preferences().inputs.select_mouse = 'LEFT';
            ...
            shutil.copy(os.path.join(os.path.dirname(os.path.abspath(__file__)), "m7a_lw_mishkey.py"), path);
        ...
        # -------------------------------------------
            ...
            if   (name == "M7A-7913-MESH-8755933"): m7a_script_add_seam(self.value);
            elif (name == "M7A-7913-MESH-8755934"): m7a_script_add_jeans_seam();
            ...
            if (name != ""):
                ...
                p_obj = bpy.data.objects[name]; p_obj.hide = False;
                p_obj.location = context.scene.cursor_location;
                
                bpy.ops.object.select_all(action='DESELECT');
                
                a_obj.select = True; p_obj.select = True;
...
def update_view3d(self, context):
    bpy_preferences("addons", "megabag").m7a_megabag_props.view3d_tools = \
    bpy_preferences("addons", "megabag").m7a_megabag_props.view3d_tools;
...
def update_view3d_tools(self, context):
    if (bpy_preferences("addons", "megabag").m7a_megabag_props.view3d_tools) and \
        (bpy_preferences("addons", "megabag").m7a_megabag_props.view3d_update):
        m7a_view3d_panels.register();
    else: m7a_view3d_panels.unregister();
...
class M7A_MEGABAG_Properties(PropertyGroup):
    ...
    view3d_navigate    = BoolProperty(default=False);
    view3d_tools       = BoolProperty(default=True, update=update_view3d_tools);
...
class M7A_MEGABAG_Preferences(AddonPreferences):
    ...
    def draw(self, context):
        def template_item(lc_cont, text, icon, data = None, props= None):
            if (ver_less(2,76,5)):
                lc_cont = lc_cont.box().column(align=False);
                lc_row = lc_cont.row(align=False);
                lc_row.label(text=text, icon=icon);
                if (data != None): lc_row.prop(data, props,  text="");
                return lc_cont.box().column(align=False);
            else:
        ...
        if (is_mishkey_exist):
            lc_btn = lc_box.operator("m7a.megabag_button", text="", icon="X");
            lc_btn.option = "UNINSTALL_MISHKEY";
            
        lc_box.separator();
        lc_btn = lc_box.operator("m7a.megabag_button", text="Set Props", icon="PREFERENCES");
        ...
        if (ver_less(2,79,5)):
            lc_box_row.prop(self.m7a_megabag_props, "view3d_tools",  text="Update Tools");
            lc_box_row.prop(self.m7a_megabag_props, "view3d_navigate",  text="Navigate Gizmos");
        ...
        lc_box_row.prop(self.m7a_megabag_props, "text_editor_info",  text="Info"  if (context.region.width < 450) else "Info Panel");
        lc_box_row.prop(self.m7a_megabag_props, "text_editor_file",  text="File"  if (context.region.width < 450) else "File Panel");
        lc_box_row.prop(self.m7a_megabag_props, "text_editor_icons", text="Icons" if (context.region.width < 450) else "Icon Viewer");
